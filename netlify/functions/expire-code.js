...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (records.length === 0) return { statusCode: 404, body: JSON.stringify({ success: false, message: 'Código no encontrado para expirar' }) };
        await table.update(records[0].id, { "Usado": true });
        return { statusCode: 200, body: JSON.stringify({ success: true, message: 'Código expirado con éxito.' }) };
